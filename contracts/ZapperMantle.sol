...
contract ZapperMantle is IZapper {
    ...
    //selector: function selector from native
    //callData:
    struct ZapOutData {
        address target;
        address token0;
    ...
    function zapInBase(bytes calldata data) external payable override {
        ...
        require(msg.value == wrapped.balanceOf(address(this)));
        //3. swap on Native.
        // if swap 0->1, approve 0 to Native.
        if (zapData.zeroForOne) {
            require(zapData.token0 == address(wrapped));
            IERC20Upgradeable(zapData.token0).approve(zapData.target, zapData.amount);
        } else {
            require(zapData.token1 == address(wrapped));
    ...
    function zapOutBase(bytes memory data) external override {
        ...
        if (zapData.zeroForOne) {
            ...
            if (zapData.selector == 0xc7cd9748) {
                newDataFinal = abi.encodeWithSignature(
                    "exactInputSingle((bytes,address,uint256,uint256,(address,address,uint256),bytes,bytes[]))",
                    updatedStruct
                );
            } else {
                bytes memory newDataFinal = abi.encodeWithSignature(
                    "exactInput((bytes,address,uint256,uint256,(address,address,uint256),bytes,bytes[]))",
                    updatedStruct
                );
            }
